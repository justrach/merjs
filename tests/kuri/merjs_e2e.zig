/// merjs E2E test suite — vendored from justrach/kuri for local CI overrides.
///
/// GitHub Actions copies this file into `kuri/src/test/merjs_e2e.zig` before
/// building the Kuri checkout, so merjs can evolve the E2E contract locally
/// without waiting on a separate Kuri repo change.
///
...
///   3. kuri on BROWDIE_PORT (default 8080) with CDP_URL set
///
...
pub fn main() !void {
    ...
    std.debug.print("\n\x1b[1mmerjs E2E — via kuri\x1b[0m\n\n", .{});

    std.debug.print("discover\n", .{});
    const discover = httpGet(a, BROWDIE_PORT, "/discover") catch |err| {
        std.debug.print("\x1b[31mERROR: kuri not reachable on port {d}: {s}\x1b[0m\n", .{ BROWDIE_PORT, @errorName(err) });
        std.process.exit(1);
    };
    s.expectContains(discover, "\"discovered\"", "kuri /discover OK");
