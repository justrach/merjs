...
pub const meta: mer.Meta = .{
    ...
    .extra_head = "<link rel=\"preload\" href=\"https://unpkg.com/leaflet@1.9.4/dist/leaflet.js\" as=\"script\">" ++
...
const html =
    ...
    \\    if(!res.ok){ console.warn('Fetch error:', url, res.status); return null; }
    ...
    \\
    \\  // National forecast
    \\  if(forecastRes?.data?.records?.length){
    ...
    \\
    \\  // 4-day outlook
    \\  if(outlookRes?.data?.records?.length){
