...
pub fn main() !void {
    ...
    for (entries.items) |path| {
        ...
        try buf.print(alloc, "    .{{ .path = \"{s}\", .render = {s}.render, .render_stream = if (@hasDecl({s}, \"renderStream\")) {s}.renderStream else null, .meta = if (@hasDecl({s}, \"meta\")) {s}.meta else .{{}}, .prerender = if (@hasDecl({s}, \"prerender\")) {s}.prerender else false, .revalidate = if (@hasDecl({s}, \"revalidate\")) {s}.revalidate else 0 }},\n", .{ url, ident, ident, ident, ident, ident, ident, ident, ident, ident });
    }
    try buf.appendSlice(alloc, "};\n\n");
