...
```bash
# 1. Generate routes (scans pages/ and api/)
zig build codegen
...
```
pages/counter.zig  →  native binary (SSR, served by Zig HTTP server)
...
The watcher polls `pages/` every 300ms, detects mtime changes, and broadcasts an SSE event to connected browsers. The browser reloads. No webpack, no esbuild, no separate watch process — just polling in a Zig thread.
...
```
merjs/
├── build.zig           # build system — scans pages/ and api/, wires modules
...
└── tools/
    ├── codegen.zig     # scans pages/ + api/, writes routes.zig
