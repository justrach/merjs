...
pub const routes: []const Route = &.{
    ...
    .{ .path = "/connected",   .render = app_connected.render, .meta = if (@hasDecl(app_connected, "meta")) app_connected.meta else .{}, .prerender = if (@hasDecl(app_connected, "prerender")) app_connected.prerender else false },
    ...
    .{ .path = "/login",       .render = app_login.render,     .meta = if (@hasDecl(app_login,     "meta")) app_login.meta     else .{}, .prerender = if (@hasDecl(app_login,     "prerender")) app_login.prerender     else false },
...
comptime {
    ...
    if (!@hasDecl(app_connected, "meta")) @compileError("app/connected.zig must export pub const meta: mer.Meta");
    ...
    if (!@hasDecl(app_login,     "meta")) @compileError("app/login.zig must export pub const meta: mer.Meta");
